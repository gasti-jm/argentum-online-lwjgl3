...
public class InquiryCommand implements CommandHandler {
    ...
    public void handle(CommandContext context) throws CommandException {
        if (context.getArgumentCount() == 0) writeInquiry();
            ...
            if (validator.isValidNumber(context.getArgumentsRaw(), NumericType.BYTE))
                writeInquiryVote(Integer.parseInt(context.getArgumentsRaw()));
            else
                console.addMsgToConsole(new String("To vote for an option write: /encuesta <option>".getBytes(), StandardCharsets.UTF_8), false, true, new RGBColor());
        }
    }
