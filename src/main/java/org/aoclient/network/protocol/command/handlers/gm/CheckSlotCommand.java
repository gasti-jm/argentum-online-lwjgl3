        ...
        if (context.hasArguments()) {
            ...
            if (tmpArr.length == 2) {
                if (validator.isValidNumber(tmpArr[1], NumericType.BYTE)) writeCheckSlot(tmpArr[0], Integer.parseInt(tmpArr[1]));
                    ...
                    console.addMsgToConsole(new String("Incorrect format. Usage: /slot <nick> <slot>".getBytes(), StandardCharsets.UTF_8), false, true, new RGBColor());
            } else
                console.addMsgToConsole(new String("Incorrect format. Usage: /slot <nick> <slot>".getBytes(), StandardCharsets.UTF_8), false, true, new RGBColor());
        } else
            console.addMsgToConsole(new String("Missing arguments. Usage: /slot <nick> <slot>".getBytes(), StandardCharsets.UTF_8), false, true, new RGBColor());
    }
